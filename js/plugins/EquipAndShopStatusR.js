//
//  装備&ショップステータス改造 ver1.01
//
// author yana
//

var Imported = Imported || {};
Imported['EquipAndShopStatusR'] = 1.01;

/*:
 * @plugindesc ver1.01/装備&ショップシーンのステータス表示を改造します。
 * @author Yana
 * 
 * @param Effect Name
 * @desc 有効度の名称です。
...
(function(){
	...
	Window_EquipStatus.prototype.refreshContents = function() {
		...
		if (this.maxPage() > 1){
			for (var i=0;i<this.maxPage();i++){
				...
				this.contents.fillRect((this.contentsWidth() - 12) - (14 * this.maxPage()) + i*14,118,12,12,color);
			}
		}
	...
	Window_RefineShopStatus.prototype.drawItemStatus = function(xx, yy) {
    	...
    	if (this.maxPage() > 1){
    		for (var i=0;i<this.maxPage();i++){
				var color = this._pageIndex === i ? 'rgba(192,255,192,1.0)' : 'rgba(0,0,0,0.5)';
				this.contents.fillRect((this.contentsWidth() - 12) - (14 * this.maxPage()) + i*14,48,12,12,color);
			}
		}
	...
	Window_RefineShopStatus.prototype.drawEffectText = function(text1,text2,x,y){
		var w = this.contentsWidth() - this.basicTextWidth()-12;
    	this.changeTextColor(this.systemColor());
	...
	/*
	Window_RefineShopStatus.prototype.nextPage = function() {
		if (this.maxPage() > 1){ SoundManager.playCursor() }
		this._pageIndex = (this._pageIndex + 1) % this.maxPage();
		this.refresh();
	};
	
	Window_RefineShopStatus.prototype.prevPage = function() {
		if (this.maxPage() > 1){ 
			SoundManager.playCursor();
			}
		this._pageIndex = (this._pageIndex + (this.maxPage() - 1)) % this.maxPage();
